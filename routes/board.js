...
router.get('/read/:idx',function(req,res,next)
{
    ...
    pool.getConnection(function(err,connection)
    {
        ...
        connection.query(sql1,[idx],function(err,result)
        {
            console.log(result);
            if(err) console.error("글 조회수 증가 중 에러 발생 err : ",err);

            if(result.affectedRows == 0)
            {
                res.send("<script>alert('조회한 글 아이디가 일치하지 않거나, 잘못된 요청으로 인해 값이 변경되지 않았습니다.');history.back();</script>");
            }
        ...
        // 글읽기
        var sql2 = "select idx, creator_id, title, content, date_format(modidate,'%Y-%m-%d %H:%i:%s') modidate, hit from board where idx=?";
        connection.query(sql2,[idx], function(err,row)
        {
